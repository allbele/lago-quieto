...
- **DECISÃO DO USUÁRIO (26/08): a loja NÃO fecha nunca e NUNCA fica por cima do jogo.** Sem botão de recolher, sem trilho, sem `shopToggle`; remover `#btn-shop` da barra em idle (ou torná-lo inerte). Desktop/landscape ≥900 px: `#shop` fixo à direita, 360 px (clamp 340-380). Telas estreitas (<900 px) ou retrato: **tela dividida vertical** — lago em cima (≥55% da altura, mínimo 300 px), loja embaixo com scroll; nunca overlay.
- **Cena não coberta**: CSS var `--shop-w` (360 em landscape / 0 em split) e `--shop-h` (0 / altura da loja em split) no body; `#lake{right:var(--shop-w); bottom:var(--shop-h)}`; `resize()` usa `canvas.clientWidth/clientHeight`; `#ui/#collection/#toasts` posicionados dentro da área do lago. `engine.refreshShopW()` retorna 0 (loja nunca cobre o canvas); `lakePoint()` usa a área toda do canvas.
